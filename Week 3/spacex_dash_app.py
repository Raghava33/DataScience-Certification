...
def get_scatterplot(sitesl,payloadsl):
    
    if sitesl == 'ALL':
        new_df = spacex_df
        new_df2 = new_df[new_df["Payload Mass (kg)"] >= payloadsl[0]]
        new_df3 = new_df2[new_df["Payload Mass (kg)"] <= payloadsl[1]]
        fig2 = px.scatter(new_df3, y="class", x="Payload Mass (kg)", color="Booster Version Category")
    else:
        new_df = spacex_df[spacex_df["Launch Site"] == sitesl]
        new_df2 = new_df[new_df["Payload Mass (kg)"] >= payloadsl[0]]
        new_df3 = new_df2[new_df["Payload Mass (kg)"] <= payloadsl[1]]
        fig2 = px.scatter(new_df3, y="class", x="Payload Mass (kg)", color="Booster Version Category")
    return fig2
